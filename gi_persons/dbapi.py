...
class PostgresPersonStore:
    ...
    def insert_person(self, connection, person, link, actor):
        with connection.cursor() as cur:
            cur.execute("""insert into public.persons
            ...
            cur.execute("""insert into public.person_organization_links
    ...
    def insert_identifier(self, connection, identifier, expected_version, actor):
        with connection.cursor() as cur:
            cur.execute("update public.persons set version=version+1,updated_at=timezone('utc',now()),updated_by=%s where organization_id=%s and person_id=%s and version=%s returning version", (actor,identifier.organization_id,str(identifier.person_id),expected_version))
            row=cur.fetchone()
            if row is None: raise VersionConflictError()
            try:
                cur.execute("""insert into public.person_identifiers
